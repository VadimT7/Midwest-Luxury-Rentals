...
import { vehicleService, bookingService, userService, prisma } from '@valore/database'
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Calculate revenue change (compare current month vs previous month)
    const now = new Date()
    ...
    const currentMonthRevenue = parseFloat(String(thisMonthRevenue._sum.amount || '0'))
    const previousMonthRevenue = parseFloat(String(lastMonthRevenue._sum.amount || '0'))
