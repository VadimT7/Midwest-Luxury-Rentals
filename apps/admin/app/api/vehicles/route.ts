...
export async function GET(request: NextRequest) {
  try {
    ...
    const transformedVehicles = await Promise.all(vehicles.map(async (vehicle: any) => {
      ...
      // Get bookings for this vehicle in the last 30 days
      ...
      const totalDays = 30
      const bookedDays = recentBookings.reduce((sum: number, booking: any) => {
      ...
      // Calculate utilization percentage and days
      const utilizationPercentage = Math.min((bookedDays / totalDays) * 100, 100)
      const utilizationDays = Math.round(bookedDays)
