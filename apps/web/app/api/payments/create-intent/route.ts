...
export async function POST(request: NextRequest) {
  try {
    ...
    let systemCurrency = 'cad' // Default fallback
    try {
      // @ts-ignore - SystemSettings model will be available after prisma generate
      const settingResponse = await prisma.systemSettings?.findUnique({
        where: { key: 'payment_currency' }
      })
      if (settingResponse?.value) {
        systemCurrency = settingResponse.value.toLowerCase()
      }
    } catch (error) {
      console.log('Using default currency CAD:', error)
    }
